...
spark = SparkSession.builder \
    .appName("NYC Taxi GBT Regressor") \
    .getOrCreate()
...
# Train the GBT Regressor and perform hyperparameter tuning
gbt = GBTRegressor(labelCol="total_amount", featuresCol="features", maxIter=10, seed=42)
...
cross_validator = CrossValidator(estimator=gbt, estimatorParamMaps=param_grid, evaluator=RegressionEvaluator(predictionCol="prediction", labelCol="total_amount", metricName="rmse"), numFolds=5)
...
# Save the model
model_path = "../models/GBTRegressor"
...
plt.legend()
plt.tight_layout()
plt.savefig("../plots/GBT_Regressor_scatter.png")
plt.show()